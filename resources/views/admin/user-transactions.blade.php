	...
	<table class="table table-bordered table-hover table-striped">
		...
		if(count($transactions)>0){
			...
			foreach( $transactions as $transaction ){
				...
				$username = $ViewUserTransactionsModel->Index($transaction->userid)->username;
				
			?>
				<tr>
					<td><?php echo $i; ?></td>
					<td><a href="{{ url( 'admin/single-transaction/'.$transaction->id ) }}" title="View Detail" >{{ $transaction->transaction_id }}</a></td>
					<td>{{ $transaction->created_on }}</td>
					<td><a target="_blank" href="{{ url('admin/user/'.$transaction->userid) }}" >{{ $username }}</a></td>
